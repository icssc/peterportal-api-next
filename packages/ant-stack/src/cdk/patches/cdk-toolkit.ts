...
import { Bootstrapper, BootstrapEnvironmentOptions } from "aws-cdk/lib/api/bootstrap/index.js";
import {
  CloudAssembly,
  DefaultSelection,
  ...
  StackCollection,
  StackSelector,
} from "aws-cdk/lib/api/cxapp/cloud-assembly.js";
import { CloudExecutable } from "aws-cdk/lib/api/cxapp/cloud-executable.js";
import {
  environmentsFromDescriptors,
  globEnvironmentsFromStacks,
  looksLikeGlob,
} from "aws-cdk/lib/api/cxapp/environments.js";
...
import { findCloudWatchLogGroups } from "aws-cdk/lib/api/logs/find-cloudwatch-logs.js";
import { CloudWatchLogEventMonitor } from "aws-cdk/lib/api/logs/logs-monitor.js";
import { StackActivityProgress } from "aws-cdk/lib/api/util/cloudformation/stack-activity-monitor.js";
import { printSecurityDiff, printStackDiff, RequireApproval } from "aws-cdk/lib/diff.js";
import { ResourceImporter } from "aws-cdk/lib/import.js";
import {
  data,
  debug,
  error,
  ...
  warning,
  withCorkedLogging,
} from "aws-cdk/lib/logging.js";
import { deserializeStructure, serializeStructure } from "aws-cdk/lib/serialize.js";
import { Configuration, PROJECT_CONFIG } from "aws-cdk/lib/settings.js";
import { numberFromBool, partition } from "aws-cdk/lib/util/index.js";
import { validateSnsTopicArn } from "aws-cdk/lib/util/validate-notification-arn.js";
import { WorkGraphBuilder } from "aws-cdk/lib/util/work-graph-builder.js";
import { AssetBuildNode, AssetPublishNode, StackNode } from "aws-cdk/lib/util/work-graph-types.js";
import { Concurrency, WorkGraph } from "aws-cdk/lib/util/work-graph.js";
...
export class CdkToolkit {
  ...
  public async diff(options: DiffOptions): Promise<number> {
    ...
    if (options.templatePath !== undefined) {
      ...
      const template = deserializeStructure(await fs.readFile(options.templatePath, "utf8"));
      ...
      for (const stack of stacks.stackArtifacts) {
        ...
        const currentTemplate = await this.props.deployments.readCurrentTemplateWithNestedStacks(
          stack,
    ...
    const deployStack = async (assetNode: StackNode) => {
      ...
      if (Object.keys(stack.template.Resources || {}).length === 0) {
        ...
        if (!(await this.props.deployments.stackExists({ stack }))) {
          warning(
            "%s: stack has no resources, skipping deployment.",
            chalk.bold(stack.displayName)
          );
        } else {
          warning(
            "%s: stack has no resources, deleting existing stack.",
            chalk.bold(stack.displayName)
          );
          await this.destroy({
      ...
      try {
        ...
        for (const name of Object.keys(result.outputs).sort()) {
          ...
          print(
            ...
            chalk.cyan(stack.id),
            chalk.cyan(name),
            chalk.underline(chalk.cyan(value))
          );
        }
  ...
  public async watch(options: WatchOptions) {
    ...
    chokidar
      ...
      .on("ready", async () => {
        latch = "open";
        debug(
          "'watch' received the 'ready' event. From now on, all file changes will trigger a deployment"
        );
      ...
      .on(
        "all",
        async (event: "add" | "addDir" | "change" | "unlink" | "unlinkDir", filePath?: string) => {
          if (latch === "pre-ready") {
            print(
              `'watch' is observing ${
                event === "addDir" ? "directory" : "the file"
              } '%s' for changes`,
              filePath
            );
          } else if (latch === "open") {
  ...
  public async import(options: ImportOptions) {
    ...
    if (stacks.stackCount > 1) {
      throw new Error(
        `Stack selection is ambiguous, please choose a specific stack for import [${stacks.stackArtifacts
          .map((x) => x.id)
          .join(", ")}]`
      );
    }
    ...
    const { additions, hasNonAdditions } = await resourceImporter.discoverImportableResources(
      options.force
    );
    ...
    print(
      `Import operation complete. We recommend you run a ${chalk.blueBright(
        "drift detection"
      )} operation ` +
    ...
    if (actualImport.importResources.length < additions.length) {
      print("");
      warning(
        `Some resources were skipped. Run another ${chalk.blueBright(
          "cdk import"
        )} or a ${chalk.blueBright(
          "cdk deploy"
        )} to bring the stack up-to-date with your CDK app definition.`
      );
    } else if (hasNonAdditions) {
      print("");
      warning(
        `Your app has pending updates or deletes excluded from this import operation. Run a ${chalk.blueBright(
          "cdk deploy"
        )} to bring the stack up-to-date with your CDK app definition.`
      );
    }
  ...
  public async destroy(options: DestroyOptions) {
    ...
    if (!options.force) {
      ...
      const confirmed = await promptly.confirm(
        `Are you sure you want to delete: ${chalk.blue(
          stacks.stackArtifacts.map((s) => s.hierarchicalId).join(", ")
        )} (y/n)?`
      );
      if (!confirmed) {
        return;
  ...
  public async list(
    selectors: string[],
    options: { long?: boolean; json?: boolean } = {}
  ): Promise<number> {
  ...
  public async synth(
    ...
    if (isIntegMode) {
      data(
        serializeStructure(
          stacks.stackArtifacts.map((s) => s.template),
          json ?? false
        )
      );
    }
    ...
    print(
      `Supply a stack id (${stacks.stackArtifacts
        .map((s) => chalk.green(s.hierarchicalId))
        .join(", ")}) to display its template.`
    );
  ...
  public async bootstrap(
    ...
    if (this.props.cloudExecutable.hasApp) {
      environments.push(
        ...(await globEnvironmentsFromStacks(
          await this.selectStacksForList([]),
          globSpecs,
  ...
  private async invokeDeployFromWatch(
    ...
    const deployOptions: DeployOptions = {
      ...
      hotswap: options.hotswap,
      extraUserAgent: `cdk-watch/hotswap-${
        options.hotswap !== HotswapMode.FALL_BACK ? "on" : "off"
      }`,
